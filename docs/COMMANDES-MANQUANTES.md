...
| `RT` partiels | `RTN` (noms), `RTI`/`RTA` (itinéraire), `RTK` (billetterie), `RTG` (services), `RTR` (remarques) | **Reportés en v2 (décision Massy 06/07/2026)** — spec complète conservée dans `missions/MISSION-16.md` §Spec RT partiels ; dispatcher actuel confirmé sans collision (`RT`, `RTN`, `RT ABC123`, `RTZZ` testés) |
...
| ~~`RE`~~ **fait Mission 16 Étape 6 (07/07/2026)** | Rappeler la dernière entrée (`RE`, `RE2`) | ✅ Fait : nouvel état CORE `state.commandHistory` (historique des saisies, `RE` lui-même jamais enregistré pour éviter qu'un second `RE` ne rappelle son propre rappel) ; `RE`/`RE(\d{1,2})?` rejoue l'entrée demandée en la réaffichant puis en la ré-exécutant. `NO PREVIOUS ENTRY` si l'historique est trop court. Non-collision vérifiée avec `RF` |
