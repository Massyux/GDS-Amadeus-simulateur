...
function handleAN(state, store, cmdUpper) {
  let dateObj = null;
  let from = null;
  let to = null;
  ...
  const flights = store.searchFlights({ from, to, ddmmm, dow });

  state.lastAN = { query: { from, to, ddmmm, dow }, results: flights };
...
export async function processCommand(api, cmd) {
  ...
  if (c.startsWith("DAC")) {
    ...
    try {
      ...
      const lines = await store.decodeIata(m[1]);
      lines.forEach(print);
    } catch (e) {
      print("INVALID FORMAT");
    }
  ...
  if (c.startsWith("DAN")) {
    ...
    try {
      ...
      const lines = await store.encodeCity(text);
      lines.forEach(print);
    } catch (e) {
      print("INVALID FORMAT");
    }
  ...
  if (c.startsWith("AN") && c.length > 2) {
    const result = handleAN(state, store, c);
    if (result.error) {
      print(result.error);
      return { events, state };
    }
