...
| RT locator/nom (**ajoutée Mission 19 réduite**) | ✅ (`RT<LOCATOR>`, `RT/<NOM>`, `RT<AA><vol>/<ddMMM>-<NOM>`) | ✅ (idempotent, re-sélectionner le même PNR ne change rien d'autre) | ✅ (`PNR NOT FOUND` si rien ne correspond, `NO ACTIVE DISPLAY` pour `RT<n>`/`RT0` sans liste en attente) | ✅ (liste de similitude si plusieurs correspondances, `RT<n>` hors plage → `PNR NOT FOUND`) | ✅ (même matrice transactionnelle que `IR` — discard des segments non enregistrés, testé explicitement) | ✅ non-collision testée avec les `RT` partiels reportés (`RTN`/`RT ABC123`/`RTZZ` restent `CHECK FORMAT`) |
